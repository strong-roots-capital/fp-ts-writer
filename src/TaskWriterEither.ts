...
export const map: <A, B>(
  f: (a: A) => B,
) => <E>(fa: TaskWriterEither<E, A>) => TaskWriterEither<E, B> =
  (f) => (fa) => async () =>
    Promise.resolve()
      .then(fa)
...
export const tellI: <E, A>(
  fm: (a: A) => string,
) => (fa: TaskWriterEither<E, A>) => TaskWriterEither<E, A> =
  (fm) => (fa) => async () =>
    Promise.resolve()
      .then(fa)
      .then(([ma, a]) =>
...
) => (fa: TaskWriterEither<E1, A>) => TaskWriterEither<E1 | E2, B> =
  (f) => (fa) => async () =>
    Promise.resolve()
      .then(fa)
      .then(async ([ma, a]) =>
...
) => (fa: TaskWriterEither<E1, A>) => TaskWriterEither<E2, A | B> =
  (onLeft) => (fa) => async () =>
    Promise.resolve()
      .then(fa)
      .then(([m, v]) =>
...
export const foldW =
  ...
  (fa: TaskWriterEither<E, A>): T.Task<B | C> =>
  async () =>
    Promise.resolve()
      .then(fa)
      .then(
...
export const fromTaskEither: (
  m: string,
) => <E, A>(fa: TE.TaskEither<E, A>) => TaskWriterEither<E, A> =
  (m) => (fa) => async () =>
    Promise.resolve()
      .then(fa)
      .then((a) => [[m], a])
